...
(define (encrypt plaintext key)
 (enc (getNums (string->list plaintext)) key))

(define (getNums list)
  (if (equal? list empty)
      '()
      (cons (char->integer (car list)) (getNums (cdr list)))))

(define (enc list key)
  (if (equal? list empty)
      '()
      (cons (mod-exp (car list) (car (cdr key)) (car key)) (enc (cdr list) key))))
...
(define (decrypt ciphertext key)
  (list->string (getLetters (dec ciphertext key))))

(define (dec list key)
  (if (equal? list empty)
      '()
      (cons (mod-exp (car list) (car (cdr key)) (car key)) (dec (cdr list) key))))

(define (getLetters list)
  (if (equal? list empty)
      '()
      (cons (integer->char (car list)) (getLetters (cdr list)))))

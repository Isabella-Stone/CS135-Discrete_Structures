...
#|-------------------------------------------------------------------------------
 | Name: Isabella Stone
 | Pledge: I peldge my honor that I have abided by the Stevens Honor System.
 |-------------------------------------------------------------------------------|#
...
(define (reflexive? n R)
  (if (null? R)
      #f
  (if (and (equal? n (car (car R))) (equal? n (car (cdr (car R)))))
      #t
      (reflexive? n (cdr R)))))
...
;; 2 PTS
(define (symmetric? R)
  (if (set-equal? (make-set (append (converse R) R)) R) #t #f))
...
(define (injective? R)
  (if (null? R)
      #t
      (if (equal? (get-range R) (make-set (get-range R))) #t #f)))

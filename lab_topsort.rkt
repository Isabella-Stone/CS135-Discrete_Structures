...
(define (helper G G-verts)
  (if (equal? G-verts '())
      (topsort-error)
      (if (equal? (indegree (get-edges G) (car G-verts)) 0)
          (car G-verts)
          (helper G (cdr G-verts)))))
...
;; 5 PTS
(define (topsort G)
  (help G (get-vertices G)))

(define (help G G-verts)
  (if (equal? G-verts '())
      '()
      (cons (find-top G) (help (pop-vertex G (find-top G)) (cdr G-verts)))))

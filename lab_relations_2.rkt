...
(define (compose S R)
  (if (or (null? S) (null? R))
      '()
      (if (null? (relate (car (cdr (car R))) S))
          (make-set (compose S (cdr R)))
          (make-set (append (relate-all (car R) S) (compose S (cdr R)))))))

(define (relate-all pt S)
  (if (null? S)
      '()
      (if (null? (relate (car (cdr pt)) S))
      '()
      (make-set (cons (list (car pt) (car (relate (car (cdr pt)) S))) (relate-all pt (cdr S)))))))
...
(define (transitive-closure R)
  (if (or (= 1 (cardinality R)) (= 0 (cardinality R)))
      R
      (union-pow (cardinality R) R)))


(define (union-pow n R)
  (if (= n 0)
      '()
      (union (power n R) (union-pow (- n 1) R))))
...
(define (equivalence? n R)
  (and (and (set-equal? R (make-set (append (id n) R))) (symmetric? R)) (transitive? R)))
...
(define (involutive? n R)
  (if (not (symmetric? R))
      #f
      (if (and (injective? R) (surjective? n R)) #t #f)))

(define (get-range R)
  (if (null? R)
      '()
      (cons (car (cdr (car R))) (get-range (cdr R)))))

(define (get n R)
  (if (null? R)
      '()
  (if (and (or (< (car (car R)) n) (= (car (car R)) n) ) (or (> (car (car R)) 1) (= (car (car R)) 1)))
      (cons (car R) (get n (cdr R)))
      (get n (cdr R)))))

(define (surjective? n R)
  (if (and (set-equal? (get-range R) (get-range (get n R))) (set-equal? (get-range R) (get-range (id n)))) #t #f))

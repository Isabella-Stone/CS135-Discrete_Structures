...
(define (jug cA cB fA fB g)
  (cond
    [(equal? fB g) (list (list fA fB))]
    [(equal? fB cB) (cons (list fA fB) (jug cA cB fA 0 g))]
    [(equal? fA 0) (cons (list fA fB) (jug cA cB cA fB g))]
    [(> (+ fA fB) cB) (cons (list fA fB) (jug cA cB (- fA (- cB fB)) cB g))]
    [(<= (+ fA fB) cB) (cons (list fA fB) (jug cA cB 0 (+ fB fA) g))]))
...
(define (help p n)
  (if (equal? n 1)
      '()
      (cons (smallest-div 2 n) (help (smallest-div 2 n) (quotient n (smallest-div 2 n))))))
...
;; 2 PTS
(define (prime? n)
  (if (and (> n 1) (only-prime n (factorize n))) #t #f))

(define (only-prime n fact)
  (if (and (equal? (car fact) n) (prime n (- n 1)))
      #t
  (if (and (not (equal? (car fact) n)) (prime (car fact) (- (car fact) 1)))
      #f
      (only-prime n (cdr fact)))))

(define (prime n n2)
  (if (equal? 1 n2)
      #t
      (if (equal? (modulo n n2) 0)
          #f
          (prime n (- n2 1)))))
...
;; 3 PTS
(define (cong-mod? m)
  (lambda (a b) (if (divides? m (- a b)) #t #f)))

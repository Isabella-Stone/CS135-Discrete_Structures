...
(define (get-vertices G)
  (usort (append (get-1 G) (get-2 G))))

(define (get-1 G)
  (if (equal? G empty)
      '()
      (cons (car (car G)) (get-1 (cdr G)))))

(define (get-2 G)
  (if (equal? G empty)
      '()
      (cons (car (cdr (car G))) (get-2 (cdr G)))))
...
(define (degree G v)
  (if (equal? G empty)
      0
      (if (and (equal? (car (car G)) v) (equal? (car (cdr (car G))) v))
          (+ 2 (degree (cdr G) v))
          (if (or (equal? (car (car G)) v) (equal? (car (cdr (car G))) v))
              (+ 1 (degree (cdr G) v))
              (degree (cdr G) v)))))
...
;; 3 PTS
(define (all-even? G)
  (each-even? G (get-vertices G)))

(define (each-even? G Glist)
  (if (equal? Glist empty)
      #t
      (if (not (equal? (modulo (degree G (car Glist)) 2) 0))
          #f
          (each-even? G (cdr Glist)))))
...
(define (adjacent G v)
  (if (equal? G empty)
      ...
      (if (equal? (car (car G)) v)
          ...
          (if (equal? (car (cdr (car G))) v)
              (usort (cons (car (car G)) (adjacent (cdr G) v)))
              (usort (adjacent (cdr G) v))))))
...
;; 4 PTS
(define (connected? G)
  (check-each (get-vertices G) (get-vertices G) G))

(define (check-each Glist GlistX G)
  (if (equal? GlistX empty)
      #t
      (if (not (equal? Glist (usort (dfs G (car GlistX)))))
          #f
          (check-each Glist (cdr GlistX) G))))
      
  
  

#|For each vertex from (get-vertices G) and (remove L R=G) should equal(adjacent G v)----(dfs G v) |#
...
;; 2 PTS
(define (eulerian? G)
  (if (and (connected? G) (all-even? G)) #t #f))